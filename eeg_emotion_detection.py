...
PROCESSED_FEATURES_CSV_PATH = "sample_emotion_data.csv"
...
def train_model_for_demonstration(csv_path, feature_column_names):
    ...
    if len(data) < 5:
        print("⚠ WARNING: Dataset is very small! Model will be trained on all available data for demonstration purposes only.")
    ...
    #     plt.tight_layout()
    #     print("ℹ Displaying Feature Importance plot...")
    #     plt.show()
...
def predict_emotion_from_input(model, feature_column_names):
    ...
    try:
        ...
        for feature_name in feature_column_names:
            while True:
                try:
                    value_str = input(f"➡ Enter value for {feature_name}: ").strip().lower()
                    if value_str == 'quit':
                    ...
                    print("   ⚠ Invalid input. Please enter a numerical value (e.g., 15.5) or 'quit'.")
